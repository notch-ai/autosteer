...
export const ChatInterface = forwardRef<ChatInterfaceRef, ChatInterfaceProps>(
  (
    ...
    const streamingMessage = useCoreStore((state) =>
      state.activeChat ? state.streamingMessages.get(state.activeChat) : null
    );

    const currentPermissionRequest = useCoreStore((state) => {
    ...
    return (
      <div
        ...
        className="flex flex-col h-full bg-background min-w-0 overflow-hidden"
        data-chat-interface
      >
